"""DAG: Инкрементальная индексация базы знаний (Qdrant Indexing)."""
...
CONFIG: dict[str, Any] = Variable.get(
    ...
    default_var={
        ...
        # Только данные — векторная БД живёт в Qdrant (отдельный сервис)
...
with DAG(
    ...
    reindex_vector_db = KubernetesPodOperator(
        ...
        # Qdrant-том не нужен — данные персистентны на сервере Qdrant.
        # Веса модели тянутся из S3 через StorageRouter при старте скрипта.
    ...
    notify_success = SlackWebhookOperator(
        ...
        message="✅ Инкрементальная индексация завершена. База знаний Qdrant обновлена.",
