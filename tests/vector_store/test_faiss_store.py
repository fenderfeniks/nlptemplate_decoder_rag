...
@pytest.fixture
def store():
    """Возвращает чистый инстанс FAISSVectorStore с flat-индексом."""
    return FAISSVectorStore(embedding_dim=4, index_type="flat", normalize_embeddings=False)


class TestFAISSVectorStore:
    def test_invalid_index_type(self):
        """Ошибка при инициализации с неизвестным типом индекса."""
    ...
    def test_hnsw_initialization(self):
        """Проверка успешной инициализации HNSW индекса."""
    ...
    def test_insert_shape_mismatch(self, store):
        """Защита от матриц с неправильной размерностью."""
    ...
    def test_insert_length_mismatch(self, store):
        """Защита от несовпадения длины векторов и метаданных."""
    ...
    def test_insert_rollback_on_error(self, store):
        """Проверка атомарности: метаданные откатываются, если index.add падает."""
    ...
    def test_search_with_filter(self, store):
        """Итеративный over-fetch поиск с фильтрацией."""
