...
class TestLogger:
    ...
    def test_dev_environment(self, mock_getenv):
        """Проверка инициализации в dev-режиме (стандартный формат)[cite: 33]."""
        ...
        # Проверяем, что шумные логгеры подавлены[cite: 33]
        for name in NOISY_LOGGERS:
            assert logging.getLogger(name).level == logging.WARNING

    @patch("src.utils.logger.os.getenv")
    def test_prod_environment(self, mock_getenv):
        """Проверка инициализации в prod-режиме[cite: 33]."""
